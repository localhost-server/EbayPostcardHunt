...
def extract_images_using_pdfimages(pdf_path, output_folder):
    ...
    for pdf_file in os.listdir(pdf_path):
        if pdf_file.endswith(".pdf") or pdf_file.endswith(".PDF"):
            ...
            # Run the pdfimages command to extract images in JPEG format
            subprocess.run(["pdfimages", "-j", full_pdf_path, os.path.join(output_folder, os.path.splitext(pdf_file)[0])])
...
# Step 2: Extract images using pdfimages
extract_images_using_pdfimages(pdf_path, output_folder)
